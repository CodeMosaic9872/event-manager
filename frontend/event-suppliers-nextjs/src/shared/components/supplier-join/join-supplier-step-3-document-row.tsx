...
export function JoinSupplierStep3DocumentRow({
  ...
  return (
    <>
      <input ref={inputRef} type="file" accept=".jpg,.jpeg,.png,.pdf" className="sr-only" onChange={onInputChange} />
      <div className="flex w-full min-w-0 flex-col items-end gap-3 sm:max-w-none sm:flex-row sm:items-center sm:justify-end">
        ...
        >
          ...
          <span className="pointer-events-none absolute top-1/2 right-3 -translate-y-1/2">
            <Image src="/icons/linking.svg" alt="" width={20} height={10} className="h-[10px] w-5" unoptimized aria-hidden />
          </span>
        </button>
        {chipLabel && (
          <span className="inline-flex shrink-0 items-center gap-2 rounded-full border border-[#6AB7FF] bg-[#6AB7FF] px-3 py-2 text-[14px] leading-5 text-black">
            ...
            >
              <Image src="/icons/cross.svg" alt="" width={9} height={9} className="size-[9px]" unoptimized aria-hidden />
            </button>
            <span>{chipLabel}</span>
          </span>
        )}
