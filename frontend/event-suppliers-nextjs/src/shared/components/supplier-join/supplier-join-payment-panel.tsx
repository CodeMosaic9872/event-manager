...
export function SupplierJoinPaymentPanel({
  ...
  const cvvError = useMemo(() => {
    ...
    if (cvvDigits.length < 3 || cvvDigits.length > 4) return "CVV must be 3-4 digits.";
    return null;
  }, [cvvDigits]);

  const expiryError = useMemo(() => {
    if (!expiry.trim()) return null;
    if (!/^(0[1-9]|1[0-2])\/\d{2}$/.test(expiry.trim())) return "Use MM/YY format.";
    return null;
  }, [expiry]);
    ...
    <div className="flex w-full min-w-0 flex-col gap-6" style={{ fontFamily: font }}>
      <div className="flex w-full min-w-0 flex-col gap-8 rounded-[24px] border border-black bg-[rgba(134,85,246,0.06)] p-8 shadow-[0_0_20px_rgba(134,85,246,0.2)] backdrop-blur-[6px]" dir="ltr" lang="en">
        ...
        <form className="flex w-full min-w-0 flex-col gap-6" onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col gap-2">
            <label className="block text-right text-[14px] leading-5 text-black/70">Discount code</label>
            <div className="relative">
              <span className="pointer-events-none absolute top-1/2 -translate-y-1/2 inset-s-4"><LockIcon /></span>
