      ...
      <div className="mx-auto flex w-full max-w-[1120px] flex-col-reverse gap-8 lg:flex-row lg:items-start lg:gap-10 xl:gap-12">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-8 sm:gap-10 lg:gap-12">
          <section className="flex flex-col gap-4 text-right sm:gap-6">
            <h2 className="text-xl font-normal leading-7 text-[#201C44] sm:text-2xl sm:leading-8">
              {article.visionTitle}
            </h2>
            {article.visionParagraphs.map((p, i) => (
              <p key={i} className="text-base leading-7 text-[#0F172A] sm:text-lg sm:leading-[29px]">
                {p}
              </p>
            ))}
            <blockquote className="rounded-2xl border-r-4 border-[#201C44] bg-[rgba(43,58,103,0.1)] p-5 text-right text-base leading-7 text-[#334155] sm:p-8 sm:text-xl sm:leading-7">
              {article.quote}
            </blockquote>
          </section>

          <section className="flex flex-col gap-6 sm:gap-8">
            <h2 className="text-end text-xl font-normal leading-7 text-[#0F172A] sm:text-2xl sm:leading-8">{article.teamTitle}</h2>
            <div className="grid grid-cols-1 justify-items-center gap-5 sm:grid-cols-2 sm:gap-6 lg:grid-cols-3 lg:justify-items-end">
