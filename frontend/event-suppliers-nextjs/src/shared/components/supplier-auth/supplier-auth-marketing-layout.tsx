...
export function SupplierAuthMarketingLayout({
  ...
  return (
    <MarketingPageShell showBackgroundImage dir="rtl" lang="he">
      <div
        className="mx-auto flex w-full max-w-full px-4 pb-16 pt-12 sm:max-w-[480px] sm:px-0"
          ...
          <div className="flex min-h-0 flex-1 flex-col items-center" dir="ltr" lang="he">
            <div className="mb-6 w-full max-w-full sm:w-[300px] rounded-full bg-[#D3E2F5] p-1">
              <div className="grid grid-cols-2 gap-0 text-center text-[14px] leading-5">
                {activeTab === "user" ? (
                  <span className="rounded-full bg-[#201C44] py-2 text-white">{userTabLabel}</span>
                ) : (
                  <Link href={userTabHref} className="rounded-full py-2 text-black transition hover:bg-white/40">{userTabLabel}</Link>
                )}
                {activeTab === "provider" ? (
                  <span className="rounded-full bg-[#201C44] py-2 text-white">{providerTabLabel}</span>
                ) : (
                  <Link href={providerTabHref} className="rounded-full py-2 text-black transition hover:bg-white/40">{providerTabLabel}</Link>
                )}
              </div>
            </div>

            <div className="mb-6 flex w-full max-w-full flex-col items-center gap-3 text-center sm:max-w-[382px]">
              <h1 className="text-[26px] font-semibold leading-9 tracking-tight text-[#1E1B4B] sm:text-[30px]">{heading}</h1>
              <p className="text-[16px] font-normal leading-6 text-[#64748B]">{subheading}</p>
            </div>
            ...
            <div className="mb-4 flex h-[54px] w-full max-w-full flex-row items-stretch rounded-2xl border border-black/10 bg-black/10 p-1.5 backdrop-blur-sm sm:max-w-[382px]">
              <button
                type="button"
