    ...
    <MarketingModal open={open} onClose={onClose} backdrop="slate" zClass="z-[200]" closeOnBackdropClick closeOnEscape>
      <div role="dialog" aria-modal aria-labelledby="supplier-rating-title" className={`${MARKETING_ICE_SURFACE_CLASS} relative max-h-[calc(100dvh-2rem)] w-full max-w-[712px] overflow-x-hidden overflow-y-auto overscroll-contain`}>
        <div className="flex flex-col items-center gap-10 px-6 pb-10 pt-12 sm:gap-12 sm:px-[66px] sm:pb-12 sm:pt-14">
          <h2 id="supplier-rating-title" className="w-full max-w-[580px] text-center text-[48px] font-normal leading-8 tracking-[-0.6px] text-[#00113A]">דירוג ספק</h2>
          ...
          <div className="flex w-[332px] max-w-full flex-col items-stretch gap-3 pt-2">
            ...
            <button type="button" onClick={handleSubmit} disabled={isSubmitting} className="relative flex h-14 w-full items-center justify-center rounded-[99px] bg-[#201C44] text-base leading-6 text-white transition hover:bg-[#2a255b] disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6AB7FF] focus-visible:ring-offset-2">
              {isSubmitting ? "שולח..." : "שליחת ביקורת"}
            </button>
            <button type="button" onClick={onClose} className="flex h-11 w-full items-center justify-center rounded-[99px] border border-black bg-transparent text-sm leading-5 text-black transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6AB7FF] focus-visible:ring-offset-2">ביטול</button>
          </div>
        </div>
      </div>
    </MarketingModal>
