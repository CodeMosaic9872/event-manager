...
export function SupplierSimilarCard({ id, name, subtitle, rating, imageUrl }: SupplierSimilarCardProps) {
  return (
    <Link
      href={`/suppliers/${id}`}
      dir="ltr"
      className="group flex h-[270px] w-full max-w-[320px] shrink-0 flex-col items-start overflow-hidden rounded-2xl border border-black/20 bg-[#F2F5F5] backdrop-blur-[6px] sm:max-w-[262px]"
    >
      <div className="h-[160px] w-full overflow-hidden self-stretch">
        <img src={imageUrl} alt="" className="h-full w-full object-cover transition group-hover:opacity-95" />
      </div>
      <div className="flex h-[108px] w-full flex-col gap-1 p-4">
        <div className="flex h-6 w-full flex-col items-stretch self-stretch" dir="rtl">
          <h4 className="line-clamp-2 w-full text-right text-base leading-6 text-black">{name}</h4>
        </div>
        <div className="flex h-4 w-full flex-col items-stretch self-stretch" dir="rtl">
