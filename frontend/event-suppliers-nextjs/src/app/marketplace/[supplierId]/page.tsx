...
export default function SupplierProfilePage() {
  ...
  return (
    <section className="relative mx-auto w-full max-w-[1300px] overflow-hidden rounded-[24px] border border-[#bfdbfe] pb-12">
      <div className="h-[180px] bg-cover bg-center" style={{ backgroundImage: supplier.heroBannerUrl ? `url(${supplier.heroBannerUrl})` : "url('/cover.png')" }} />
      <div className="-mt-20 text-center">
        <div
          className="mx-auto size-60 rounded-full border-4 border-white bg-cover bg-center shadow-xl"
          style={{ backgroundImage: supplier.avatarUrl ? `url(${supplier.avatarUrl})` : "url('/avatars/1.jpg')" }}
        />
      ...
      <div className="mx-auto mt-6 max-w-[1100px]">
        ...
        {supplier.reviews && supplier.reviews.length > 0 && (
          <div className="mt-8 grid gap-3">
            {supplier.reviews.slice(0, 2).map((r) => (
              <div key={r.id} className="rounded-xl border border-slate-200 bg-white p-3">
                <p className="text-sm text-[#3f3f69]">{r.author}</p>
      ...
      {showRating && (
        <>
          ...
          <div className="fixed left-1/2 top-1/2 z-50 w-[700px] max-w-[95vw] -translate-x-1/2 -translate-y-1/2 rounded-[24px] bg-[#deecfd] p-8 shadow-2xl">
            ...
            <div className="mx-auto mt-5 h-36 w-36 rounded-xl bg-cover bg-center" style={{ backgroundImage: supplier.avatarUrl ? `url(${supplier.avatarUrl})` : "none" }} />
