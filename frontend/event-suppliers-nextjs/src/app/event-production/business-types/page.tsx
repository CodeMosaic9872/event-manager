...
export default function BusinessTypesPage() {
  return (
    <section className="relative mx-auto min-h-[calc(100vh-120px)] w-full max-w-[1440px] overflow-hidden rounded-[24px] border border-[#bfdbfe] bg-[linear-gradient(180deg,#9BD3EF_0%,#FFFFFF_58%)] px-6 py-10">
      <div className="pointer-events-none absolute -left-20 top-40 size-72 rounded-full bg-[#6ab7ff]/20 blur-2xl" />
      <div className="pointer-events-none absolute -right-24 bottom-4 size-72 rounded-full bg-[#6ab7ff]/30 blur-2xl" />
        ...
        {businessTypes.map((item) => (
          <Link
            ...
            className={`rounded-[24px] border border-[#c3d3df] px-8 py-6 text-center text-[48px] leading-none text-[#252a4b] ${
              item === "נופש" ? "border-[#2f71d8] bg-[#201c44] text-white shadow-[inset_0_0_0_2px_#60b5ff]" : "bg-[#eaf3fb]"
            }`}
          >
            {item}
          </Link>
      ...
      <div className="mt-14 text-right">
        <Link href="/event-production" className="inline-flex items-center gap-3 rounded-full border border-slate-400 px-8 py-3 text-xl text-[#3a4362]">
          <span>←</span> חזרה
        </Link>
      </div>
    </section>
